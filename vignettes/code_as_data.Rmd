...
Although we have used new environment, search path has been affected. The safest way to evaluate expression would be to do it in separate R session - we can do this using `{callr}` package^[Csárdi G, Chang W (2022). _callr: Call R from R_. R package version 3.7.2, <https://CRAN.R-project.org/package=callr>.].

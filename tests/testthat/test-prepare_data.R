...
test_that("'prepare_data' returns correctly prepared
  ...
  expected <- structure(list(line = list("1", "3", "5", "7\n8\n9", "11\n12\n13",
                                         "15\n16\n17", "19", "21\n22\n23", "25", "27", "29", "31\n32\n33",
                                         "35", "36"), src_code = list("library(dplyr)", "dir <- system.file(package = \"tableboom\", \"example_script\")",
                                                                      "source(paste0(dir, \"/example_script_sourced_inside.R\"))",
                                                                      "my_fun2 <- function(x) {\n  x + 2\n}", "my_fun3 <<- function(x) {\n  x + 2\n}",
                                                                      "function(x) {\n  x + 2\n} -> my_fun4", "assign(\"my_fun5\", function(x) x + 2)",
                                                                      "for (i in 1:3) {\n  cat(i)\n}", "my_df[\"x\"]", "my_df",
                                                                      "my_vec <- rep(my_df$x + 10, 2)", "my_df <- my_df %>%\n  mutate(y = mean(my_vec)) %>%\n  select(y)",
                                                                      "str(my_df)", "my_df"), inspected_src_code = list("<  >  library(dplyr) ",
  ...
  expect_s3_class(obj, c("tbl_df", "tbl", "data.frame"))
})
...
test_that("'prepare_orig_script' returns correctly prepared original source code", {
  ...
  expected <- structure(list(line = list("1", "3", "5", "7\n8\n9", "11\n12\n13",
                                         ...
                                         "35", "36"), src_code = list("library(dplyr)", "dir <- system.file(package = \"tableboom\", \"example_script\")",
                                                                      ...
                                                                      "for (i in 1:3) {\n  cat(i)\n}", "my_df[\"x\"]", "my_df",
                                                                      "my_vec <- rep(my_df$x + 10, 2)", "my_df <- my_df %>%\n  mutate(y = mean(my_vec)) %>%\n  select(y)",
                                                                      "str(my_df)", "my_df")), class = c("tbl_df", "tbl", "data.frame"
                                                                      ), row.names = c(NA, -14L))
